 ...
 * @returns 
 */
export async function queryItems(item_type: string, page: number): Promise<Item[]> {
 ...
 */
export async function getItem(id: string, returnComments = false): Promise<Item | false> {
 ...
 */
export async function getComments(items: number[], level: number = 0): Promise<Comment[]> {
    ...
    items.map(async (item_id) => {
      try {
        ...
        return {
          ...
          dead: comment.dead,
          comments: comment.kids ? await getComments(comment.kids, level + 1) : []
        } as Comment;
      } catch(err) {
        console.log(err);
        return {} as Comment;
    ...
    throw new Error('Not HTML content');
      ...
      const $ = load(body);
			...
			for (let i = 0, l = rows.length; i < l; i += 2){
				...
				let id = (voteLink && voteLink.length ? (voteLink.attr('id')?.match(/\d+/) || [])[0] : null);

				const cell1 = row1.find('td.title').has('a');
				const link = cell1.find('a:first');
				const title = link.text().trim();
				let url = link.attr('href');
				const domain = (cell1.find('.comhead').text().match(/\(\s?([^()]+)\s?\)/i) || [,null])[1];
				...
				const commentsCount = commentsCountLink && /\d/.test(commentsCountLink.text()) ? parseInt(commentsCountLink.text(), 10) : 0;
